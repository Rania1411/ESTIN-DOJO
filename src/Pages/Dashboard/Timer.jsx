...
export default function Timer({ year, setYear }) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 text-[#112250]">
      {/* Timer Card */}
      <div className="bg-[#D9CBC2]/80 rounded-3xl p-6 flex flex-col md:flex-row justify-between items-center gap-6 shadow-[0_20px_60px_rgba(0,0,0,0.3),0_1px_0_rgba(255,255,255,0.4)_inset]">
        {/* Left: Timer display */}
        <div className="flex flex-col items-center flex-1">
          ...
          <h1 className="text-6xl md:text-7xl font-bold">{formatTime(time)}</h1>
          <p className="text-[#112250]/80 mt-1 font-medium"> 
            {mode === "Pomodoro" ? "Focus session" : "Break time"}
          </p>
        </div>

        {/* Right: Year & Module selection + Start/Pause */}
        <div className="bg-[#3c507d]/80 p-4 rounded-xl w-64 border-2 border-[#112250]/40 flex flex-col gap-4">
          <div>
            <p className="text-sm text-[#f5e0e9] mb-1 font-medium">Year of study</p>
            <select
              value={year}
          ...
          <div>
            ...
            >
              ...
              {year && modulesByYear[year].map((mod) => (
          ...
          <button
            ...
            disabled={!module}
            className={`flex items-center justify-center gap-2 w-full bg-[#dfb259]/90 text-[#112250] rounded-md p-2 font-semibold hover:scale-105 transition
                ${!module 
                  ? "bg-[#f5e0e9]/60 cursor-not-allowed text-[#112250]" // style when disabled
                  : "bg-[#dfb259]/70 text-[#112250] hover:scale-105 cursor-pointer" // style when enabled"
              }`}
          >
